...
### Improved
- **Automated test coverage for the config round-trip guarantee.** A Swift Package (`Package.swift`) alongside the Xcode project makes the pure Foundation-only model layer runnable with `swift test`. 124 tests across 18 suites cover the parser, serializer, ID-preservation mechanism, ProxyJump alias extraction, semantic version comparison, host list filtering, palette search scoring, `/etc/hosts` block computation, and all sshCM metadata markers. Run with `swift test` from the repo root.
